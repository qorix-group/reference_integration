...
static char send_data[1024 * 1024 * 10];  /* Send buffer filled with pattern data */
...
kso_process_receive(void)
{
    ...
    while (kso_thread_run && counter < 16) {
        printf("kso: sending %d bytes for 10-secs...\n", tx_rx_sizes[counter]);
        bytes_transmitted = 0;
...
repeat_transmit:
        if (counter == 0) {
            ...
            if (received_size == 0)
            {
                /* MSB of first byte to determine expected size */
                expected_size = tx_rx_sizes[(recv_buf[0] & 0xF0) >> 4];

                /* LSB of first byte to determine transmit size */
                response_size = tx_rx_sizes[recv_buf[0] & 0x0F];
            }
        ...
        {
            if (response_size >= MJUM16BYTES) {
                send_length = MJUM16BYTES;
            }
            else {
             ...
             */
            m_copyback(send_mbuf, 0, send_length, send_data + offset);
        ...
        if (elapsed_time < 10000) {
            ...
            tx_duration_ms[counter] = elapsed_time;
    ...
    printf("kso: TCP Transmission Summary:\n");
    bytes_transmitted = 0;
