...
public:
    ...
    int run(const std::string& input) override;
};
...
int MultipleKvsPerApp::run(const std::string& input) {
    try {
        // Parse parameters
        ...
        // ============================================================
        {
            ...
            if (!kvs1_opt) {
                std::cerr << "Failed to create KVS instance 1" << std::endl;
                return 1;
            }
            auto kvs1 = *kvs1_opt;
            ...
            if (!kvs2_opt) {
                std::cerr << "Failed to create KVS instance 2" << std::endl;
                return 1;
            }
            auto kvs2 = *kvs2_opt;
            ...
            if (!kvs1->set_value(test_input.key, test_input.value_1)) {
                std::cerr << "Failed to set kvs1 value" << std::endl;
                return 1;
            }

            if (!kvs2->set_value(test_input.key, test_input.value_2)) {
                std::cerr << "Failed to set kvs2 value" << std::endl;
                return 1;
            }

            // Flush both instances to disk
            if (!kvs1->flush()) {
                std::cerr << "Failed to flush kvs1" << std::endl;
                return 1;
            }

            if (!kvs2->flush()) {
                std::cerr << "Failed to flush kvs2" << std::endl;
                return 1;
            }
        ...
        // ============================================================
        {
            ...
            auto kvs1_opt = KvsInstance::create(params1);
            if (!kvs1_opt) {
                std::cerr << "Failed to reopen KVS instance 1" << std::endl;
                return 1;
            }
            auto kvs1 = *kvs1_opt;
            ...
            auto kvs2_opt = KvsInstance::create(params2);
            if (!kvs2_opt) {
                std::cerr << "Failed to reopen KVS instance 2" << std::endl;
                return 1;
            }
            auto kvs2 = *kvs2_opt;
            ...
            if (!value1_opt) {
                std::cerr << "Failed to read kvs1 value" << std::endl;
                return 1;
            }
            ...
            if (!value2_opt) {
                std::cerr << "Failed to read kvs2 value" << std::endl;
                return 1;
            }
            ...
            if (!KvsInstance::normalize_snapshot_file_to_rust_envelope(params1)) {
                std::cerr << "Failed to normalize KVS snapshot output for instance 1" << std::endl;
                return 1;
            }
            if (!KvsInstance::normalize_snapshot_file_to_rust_envelope(params2)) {
                std::cerr << "Failed to normalize KVS snapshot output for instance 2" << std::endl;
                return 1;
            }
        }
...
std::shared_ptr<Scenario> make_multiple_kvs_per_app_scenario() {
    return std::make_shared<MultipleKvsPerApp>();
}
